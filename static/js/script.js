document.addEventListener('DOMContentLoaded', function() {
    ...
    if (viewPedidoModal) {
        viewPedidoModal.addEventListener('show.bs.modal', function (event) {
            ...
            // Obtener los datos del pedido de los atributos data-*
            const data = {};
            for (let i = 0; i < button.attributes.length; i++) {
                const attr = button.attributes[i];
                if (attr.name.startsWith('data-')) {
                    const key = attr.name.substring(5); // Eliminar 'data-'
                    data[key] = attr.value;
                }
            }
            ...
            if (viewImagePreview) {
                ...
                if (data.imagen_path) {
                    ...
                    img.src = '/static/' + data.imagen_path;
    ...
    // Declarar las instancias de los gráficos en un ámbito más amplio
    ...
    function renderCharts() {
        ...
        if (estadoPedidosCtx) {
            if (estadoPedidosChartInstance) {
                estadoPedidosChartInstance.destroy(); // Destruir instancia anterior si existe
            }
            ...
            estadoPedidosChartInstance = new Chart(estadoPedidosCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        borderColor: [isDarkMode ? '#1e1e1e' : '#ffffff'], // Borde blanco en modo claro, oscuro en modo oscuro
                        borderWidth: 2
                    }]
                },
                options: {
        ...
        if (ingresosMensualesCtx) {
            if (ingresosMensualesChartInstance) {
                ingresosMensualesChartInstance.destroy(); // Destruir instancia anterior si existe
            }
    ...
    function setDarkMode(enabled) {
        ...
        renderCharts(); // Volver a renderizar los gráficos con los nuevos colores
    }
    ...
    if (savedDarkMode === 'enabled') {
        ...
        setDarkMode(false); // Asegurarse de que el modo claro esté activo por defecto y los iconos correctos
    }

    // Event Listener para el botón de alternar modo oscuro
    if (darkModeToggle) {
    ...
    // Llamar a renderCharts al cargar la página (ya se hace dentro de setDarkMode)
    // renderCharts(); // No es necesario llamarlo aquí de nuevo

    // --- Lógica de Ordenación de la Tabla ---
